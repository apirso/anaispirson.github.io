...
let advMonth = 12; //testing for nov
...
//opens a door when clicked on
 for (let i = 0; i < day.length; i++) {
  day[i].addEventListener("click", function() {
    if( curMonth == advMonth && curDay >= (i+1) ) {
    day[i].classList.toggle("doorOpen");
    } else{
      alert("HO HO HO! Don't be naughty now! It's too early...Santa would not appreciate this!")
    }
     });
 }
...
let popupText = popup.children[0].childNodes[3].children[1].children[1].innerText=`${horror[str-1].description}`;
let btnLink=popup.children[0].childNodes[3].children[1].childNodes[5].href=`${horror[str-1].link}`;
...
function updateCounter(dayNo){ //specify wht counter is for
  ...
  if( curMonth == advMonth && curDay >= (dayNo) ) {
...
advType.addEventListener('change',()=>{
    if (advType.selectedIndex != 0){
        ...
        alert('classical is not ready...please watch horror')
        // setCookie('advType','classic',30)
        setCookie('advType','horror',30)
         }
...
const snowflake = new SnowflakeJs(frames=25, count=50, lifetime=100, maxSpeed=3, maxSize=25);
snowflake.init();
